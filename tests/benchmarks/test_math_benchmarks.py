...
def _make_generated(response: str) -> Generated:
    return Generated(
        case=EvaluationCase(
            prompt=Prompt(
...
def test_hmmt_accepts_whitespace_and_dfrac_variants():
    result = _make_generated(r"\boxed{\dfrac{1}{576}}")
    assert HMMT25Benchmark._is_correct(result, r"\frac{1}{576}")


def test_hmmt_accepts_boxed_answers_with_nested_braces():
    result = _make_generated(r"\boxed{1 - \frac{2}{\pi}}")
    assert HMMT25Benchmark._is_correct(result, r"1-\frac{2}{\pi}")


def test_hmmt_accepts_math_verify_equivalent_forms_when_available():
    pytest.importorskip("math_verify")
    result = _make_generated(r"\boxed{\frac{9}{\sqrt{23}}}")
    assert HMMT25Benchmark._is_correct(result, r"\frac{9 \sqrt{23}}{23}")
