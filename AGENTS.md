...
- `steptronoss/core/generators/flow_controller_simulator.py` is now the safest place to reason about new fully-async rollout rules first. Its `fully-async` path models `prompt_per_iter`, `max_untrained_prompts`, `max_staleness`, and an explicit `max_concurrent` token-level round-robin inference scheduler before the real controller exists.
