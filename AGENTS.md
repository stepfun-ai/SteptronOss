...
- DeepSeek V4 support lives in `steptronoss/model/deepseek_v4.py` with toy configs under `playground/pretrain/deepseek_v4/` and Step3-data SFT entry under `playground/sft/deepseek_v4/`. Treat it as an eager, parity-first path: TP is implemented only for attention query/output heads with replicated single-KV MQA, routed experts use EP with `ETP=1`, and PP uses the normal layer map / hyper-stream handoff. HF parity still targets the TP=PP=EP=1 `sliding_attention` + standard `moe` toy path.
- For DeepSeek V4 attention TP, replicated KV/compressor branches should pass through `tensor_parallel.copy_to_tensor_model_parallel_region(...)` before local attention use; that keeps the forward identical while all-reducing backward gradients into replicated KV/compressor/q-residual parameters.
- Pipeline p2p variable-sequence shape handshakes support non-3D tensors now; DeepSeek V4 depends on this because PP stages pass 4D hyper streams shaped `[S, B, hc_mult, H]`.
- As of 2026-05-07, this repo's `.venv` had `transformers 4.57.6` without DeepSeek V4. GPU parity used a temporary Transformers `main` source overlay plus `/tmp` `huggingface_hub>=1.3,<2`; if that dev snapshot hits `KeyError: flash_attn_interface`, patch `transformers.utils.import_utils.PACKAGE_DISTRIBUTION_MAPPING["flash_attn_interface"] = ["flash-attn"]` inside the test process.
- For DeepSeek V4 TP+EP parity, routed experts are duplicated across TP and sharded across EP, so expert parameter gradients need TP-duplicate compensation before comparing with HF or reducing over EDP; otherwise expert grads are exactly off by the TP factor.
- Do not use `step4_pretrain` resources for DeepSeek V4 Flash validation. That group is strictly off limits for this work, even when it has H200 machines. Code should refer to the release checkpoint through `/oss/opensources_model/DeepSeek-V4-Flash-Base`, which is a symlink to the mounted JFS checkpoint; do not hardcode internal JFS mount paths in repo files, and do not stage the checkpoint under `/data`.
- DeepSeek V4 Flash Base's downloaded tokenizer currently has no HF `chat_template`. For raw Step3 SFT data loss probes, encode with that DSV4 tokenizer using an explicit fallback such as `plain_roles` (`role: content`) and mask assistant content tokens; do not assume Step3.5 tokenizer/template semantics apply.
