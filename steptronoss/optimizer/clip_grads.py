...
from steptronoss.core.utils import (
...
def clip_grad_norm_fp32(
    ...
    if norm_type == inf:
        ...
        total_norm_cuda = torch.tensor(
            [float(total_norm)], device="cuda", dtype=torch.float
        )
        ...
        if norm_type == 2.0:
            ...
            if grads_for_norm:
                ...
                grad_norm = torch.tensor([0], device="cuda", dtype=torch.float)
    ...
    if clip_coeff < 1.0:
        dummy_overflow_buf = torch.tensor([0], device="cuda", dtype=torch.int)
        multi_tensor_applier(
            multi_tensor_scale, dummy_overflow_buf, [grads, grads], clip_coeff
        )
...
def count_zeros_fp32(parameters, model_parallel_group):
    ...
    total_num_zeros = torch.tensor([0], device="cuda", dtype=torch.float)
    for param in parameters:
