...
class Zero1GradientManager(GradientManager):
    """Minimal ZeRO-1 optimizer that reuses DDP grad buffers."""
    ...
    def replace_optimizer_with_fp32_shards(
            ...
            for _i, param in enumerate(param_group["params"]):
                ...
                if buffer_info["in_this_dp"]:
                    if param.is_cuda and param.dtype == torch.bfloat16:
                        ...
                        updated_param_list.append(main_param)
        ...
        updated_param_groups = [g for g in optimizer.param_groups if g["params"]]
        optimizer.param_groups = updated_param_groups
    ...
    @torch.no_grad()
    def _gather_model_params(self):
        for buffer in self._param_buffers.values():
            buffer.zero_()

        for param, info in self._param_info.items():
            if info["in_this_dp"]:
                buffer = info["dtype_buffer"]()
                buffer[info["range"]].data.copy_(param.view(-1))

        with get_timers().record("grads-reduce-scatter", log_level=2):
            for bucket_key, buffer in self._param_buffers.items():
                reduce_group = bucket_key.allreduce_group
                world_size = PM.size_of(reduce_group)
                group = PM.group_of(reduce_group)
                rank = PM.rank_in(reduce_group)
        ...
        for param, info in self._param_info.items():
            buffer = info["dtype_buffer"]()
            param.view(-1).data.copy_(buffer[info["range"]])
