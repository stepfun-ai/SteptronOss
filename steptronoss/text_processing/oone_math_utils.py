...
from steptronoss.text_processing.utils import get_runner


def repeatness_score(s: str):
    def ranks(l):
...
def is_latex_equal(str1, str2, math_mode="legacy"):
    if math_mode == "legacy":
        ...
        try:
            result = get_runner()(_is_latex_equal, str1, str2, timeout=2)
            return result
        except TimeoutError:
            return False
    elif math_mode == "math_verify":
        try:
            result = get_runner()(verify, parse(str1), parse(str2), timeout=2)
            return result
        except TimeoutError:
            return False
    else:
